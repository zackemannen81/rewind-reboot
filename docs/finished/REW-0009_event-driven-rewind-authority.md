...
- [x] Search live authority and acceptance for conflicting global-duration or
  timer-only end-condition claims.
- [x] Review ownership, relative links and fenced blocks manually.
- [x] `git diff --check` is clean.
- [x] Confirm the diff contains no C++, Blueprint, `.uasset` or `.umap` change.
...
- [x] Verify the complete documentation change.
- [x] Archive REW-0009, restore the task template and add the signed journal
  entry.
...
- [x] Conflicting-rule search. Remaining timer and duration matches in live
  authority are explicitly historical, rejected alternatives or statements of
  the non-compliant current build.
- [x] Manual authority, link and fence review. Fifteen changed Markdown files
  produced zero unresolved relative links and zero unclosed fences.
- [x] `git diff --check`.
- [x] Diff-type audit. No C++, header, Blueprint, `.uasset`, `.umap`,
  project or config file changed.
...
- Current state: Complete. ADR-0009 and all owning rules are event-driven; the build
