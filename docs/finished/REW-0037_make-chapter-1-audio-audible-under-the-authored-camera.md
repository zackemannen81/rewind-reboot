...
- [x] Run focused listener/radio tests and the complete `Rewind.*` suite.
- [x] Inspect the built/runtime class or named test state for the audio
      component and player-relative listener contract.
- [x] Run `git diff --check`, inspect the full diff and state skipped PIE or
...
- [x] Archive this task, restore the template, prepend a signed journal entry.
- [x] `git diff --check`, inspect the full diff against the ownership list,
...
- An initial focused listener run failed because the isolated `CreateWorld`
  ...
  reruns.
- Delivery: implementation commit `a41e7b2` was pushed and pull request #39
  was opened against `main`. The archive/journal finalization is its final
  explicit-path documentation checkpoint.
...
- [x] `docs/SYSTEMDOC.md`
- [x] `docs/JOURNAL.md`
- [x] `docs/FILESTRUCTURE.md` when structure changes — no map change required
- [x] ADRs when long-lived decisions change — none changed

## Handoff and Follow-ups

- Current state: complete on `codex/rew-0037-audio-listener-runtime`; runtime,
  focused and full-suite gates pass, and pull request #39 is open for review.
- Next recommended step: operator review, interactive PIE/heard-output check,
  and merge decision.
- Blockers: none.
- Child tasks: none.
