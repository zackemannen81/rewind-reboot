...
- [x] Three loops demonstrated from a clean save with `LogRewind` evidence
- [x] Loop B failure attributed to the fuse and route, not another timeout
- [x] Stairs minus lift measured at one turnstile period or more
- [x] Naive minus learned measured at one turnstile period or more
- [x] Every region declares a camera
- [x] Animation import size measured on disk
...
- [x] Implement the lift and the stairs
- [x] Build 4C, landing, stairwell and shaft, courtyard, street and hub
- [x] Declare a camera for every region
- [x] Import the Tier 1 animations and mannequin; measure the size
- [x] Measure both timing rules and adjust the layout until they hold
- [x] Demonstrate the three loops and write the playtest record
- [x] Update status, system document and file map
...
- Owner-provided concept targets now include 4C, the fuse box, the stairwell
  and the lift shaft. The owner clarified that 4C is on floor 4, so this
  implementation uses three physical stair sections and two intermediate
  landings to reach the entrance floor. This resolves blockout construction
  inside the frozen lift-or-stairs scope; it changes no game rule.
- The owner's circulation sketches further clarify the same in-scope spaces.
  The latest paired floor plans settle the construction: stairs, lift and 4C
  form the upper row above their common hallway; the apartment and stairs are
  filmed from the upper side while the hall/lift is filmed from below. On the
  entrance floor, both routes meet a long hallway whose fuse-box threshold
  opens into one large courtyard containing gate, generator, patrol yard and
  the route to Transit. The 170 m generator journey is folded around that yard
  so it remains a real branch without becoming a detached corridor. The 4C
  interaction sketch places the keypad at its hall door, radio on a central
  cabinet and carried fuse at the technical wall/fuse-box area. These are
  blockout placement and camera clarifications, not new rules or art scope.
...
- [x] Three-loop demonstration from a clean save
- [x] Loop B failure attributed to the fuse and route
- [x] Both timing measurements
- [x] Region enumeration
- [x] Import size on disk
- [x] Manual link and fence review
- [x] `git diff --check`
- [x] Document skipped checks and reasons

Editor Win64 Development built successfully with UE 5.8. All six named
`Rewind.*` automation tests passed together. The formal clean-save run is
recorded in `docs/playtests/chapter-1-three-loops-2026-08-23.md`: stairs minus
lift measured 49.67 seconds, Loop B timed out at 240 seconds without hub entry
and Loop C entered at 90.67 seconds. Eleven camera regions enumerated complete.
The Tier 1 import is 31 assets and 12.61 MiB. No packaged build was made and
frame rate was not varied.
...
- [x] A new record under `docs/playtests/`
- [x] `docs/backlog/character-animation-source.md` when resolved

## Handoff and Follow-ups

- Current state: Complete. The authored chain, measured spatial blockout,
  cameras, character subset and three-loop evidence are delivered.
- Next recommended step: no successor task is approved. The owner must choose
  whether to activate bounded visual/art-direction work against the nine
  concept references or first commission a player-experience test of the
  proven blockout; REW-0007 does not authorize either implementation.
- Blockers: none.
- Child tasks: REW-0008 complete and archived.
- Resume condition: met.
- Verification gaps: packaged build and varied-frame-rate testing were outside
  the completed evidence; enjoyment and finished visual fidelity remain
  untested and outside this charter.
- Open questions: none.
