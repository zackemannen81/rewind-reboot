...
- [x] Three PIE runs covering the three FL-17 outcomes, with held keys
      released before every stop.
- [x] A clean viewport capture showing the interference during a prelude.
...
- [x] Implement `ARewindAuthoredCourtyard` blockout, regions, gameplay and
      threshold checkpoint, attached to `GroundHall_Region` without a gap.
- [x] Spawn that courtyard on `Rewind.SkipProofLayout` maps only; keep the
      procedural proof's GroundFuseGate at X = 4110.
- [x] Relocate the existing courtyard/ground fuse socket to the authored
      threshold; wire generator to gate on the authored map.
- [x] Named `Rewind.*` tests for the new spatial constants and region abutment.
- [x] Developer command so the Anchor pass can be evidenced without FL-18.
- [x] Editor-closed `RewindEditor Win64 Development` build.
- [x] Headless `Rewind.*` automation together.
- [x] Three FL-17 PIE runs and an interference capture; release held keys.
- [x] Update `CURRENT_STATUS.md`, `SYSTEMDOC.md`, `FILESTRUCTURE.md` if needed,
...
- [x] Editor-closed build succeeds. Final confirm: Result Succeeded, 1.78 s,
      target up to date.
- [x] All discoverable Rewind.* tests pass together, run headless. 22/22
      Success, `TEST COMPLETE. EXIT CODE: 0`, including
...
- [x] Three PIE runs covering the three FL-17 outcomes, with held keys
      released before every stop. Record:
      `docs/playtests/authored-courtyard-fl17-2026-08-25.md`.
- [x] A clean viewport capture showing the interference during a prelude:
...
- [x] `docs/playtests/` index and FL-17 record
- [ ] ADRs when long-lived decisions change

## Handoff and Follow-ups

- Current state: complete. Authored courtyard attached at GroundHall +Y;
  GroundFuseGate at `(-430, 2700, 150)`; three FL-17 PIE outcomes recorded.
- Next recommended step: owner review of the courtyard first-pass look and
  of FL-18 as a separate commit-boundary playtest.
- Blockers: none
- Child tasks: none
- Resume condition: not paused
- Open questions: 4C fuse at X `-250` still sits outside Apartment4C
  `X [-245, 370)` beside the building socket; that blocked this-loop pickup
  in PIE and is not decided here. Plaza floor is over-bright under the
  first-pass moon key.
