...
Superseded by: REW-0006, 2026-08-23
...
- Current state: **Resumed** 2026-08-23. REW-0005 is complete, so the
  resume condition is met and this charter is restored unchanged.
- **Superseded 2026-08-23 by REW-0006.** This charter's out-of-scope list
  excludes "branching routes, alternate paths and route knowledge", and
  REW-0005 decided a lift-or-stairs choice, which is a branch. The
  exclusion was written before the chain existed. Reading it narrowly to
  make it fit would be the tightening a freeze exists to prevent, so the
  charter is superseded rather than reinterpreted.
- The goal was not wrong and is carried forward unchanged into REW-0006:
  an authored space played through a camera the author controls, in which
  a wasted loop costs the player something measurable.
- What this task delivered and REW-0006 does not repeat: the camera grammar
  design document, the authored walk speed and loop duration, the timing
  form of the knowledge-saving rule, `ARewindCameraRegion` and
  `ARewindCameraRig`, the character stripped of its camera, movement
  expressed against the frame, and camera-side walls made collision-only.
  All merged to `main`.
...
- Resume condition: met. `chapter-1-authored.md` now states the chain,
  including the fuse's two sockets, the lift, the stairs and the radio's
  cost.
- Next recommended step: settle the scope conflict above, then lay out the
  rebuild against the chain. The connective space this charter allows is
  the stairwell and lift shaft between 4C and the courtyard, which the
