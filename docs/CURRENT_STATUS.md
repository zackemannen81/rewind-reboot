...
| `Rewind/Source/Rewind/RewindLift.cpp` and `RewindStairwell.cpp` | A physical bidirectional six-second cage journey between floor 4 and entrance, plus the always-available stair routes. The authored cage has a blocking floor, centres its passenger and hands the capsule to the hall side of each coplanar threshold. Final REW-0012 PIE measured 6.00 s down and 6.00 s up |
