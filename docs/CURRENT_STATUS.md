...
| `docs/adr/ADR-0011_asset-source-and-provenance.md` | Accepted and implemented by REW-0022. Art assets are project-owned or generated; third-party packs stay outside the tracked closure. The authored default map's editor dependency set contains no `/Game/Fab/` or `/Game/Art/Texture/` package |
...
| `docs/design/` | Eight accepted documents. Ownership in `docs/design/README.md`; `player-messages.md` owns the player-facing text channel and states that the debug overlay is not UI |
...
| `docs/CURRENT_TASK.md` | Restored task template. REW-0016 to REW-0019, REW-0021 and REW-0022 are complete and archived; no task is active on this branch |
...
| `AutomationTestToolset` | Enabled. Seventeen `Rewind.*` tests exist after REW-0017 and REW-0018: the nine inherited, plus `Rewind.Loop.Prelude.Bounds`, `Rewind.Loop.Prelude.Latch`, `Rewind.Loop.AnchorCommit.Boundary`, `Rewind.Loop.BreakSignature.ReadsLoopTime`, `Rewind.Message.Queue.Queueing`, `Rewind.Message.Queue.Ordering`, `Rewind.Message.Queue.Expiry` and `Rewind.Message.Catalog.ResolvesSliceCopy`. All seventeen were run together on the merged tree by the REW-0016 operator on 2026-08-25: 17 passed, 0 failed, `TEST COMPLETE. EXIT CODE: 0`. Each agent had previously verified only its own four, where the others did not yet exist |
...
| `Rewind/Source/Rewind/RewindMessageSubsystem.cpp` | Player-facing text. Actors speak by catalog id; copy lives in `FRewindMessageCatalog`; one Slate line sits at the bottom of the viewport. The engine debug overlay is not this channel. `RewindLoopSubsystem` still draws `t=` |
