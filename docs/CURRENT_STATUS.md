...
| `Rewind/Rewind.uproject` | Unreal Engine 5.8 C++ project. Loop clock, apply order, session save, CleanSave. Loops end on causal-contract failure, death or first-time Anchor commit. The 240-second timeout is no longer the default end condition |
...
| `docs/CURRENT_TASK.md` | Restored task template. REW-0017 and REW-0018 are complete and archived; no task is active on this branch |
...
| `AutomationTestToolset` | Enabled. Seventeen `Rewind.*` tests exist after REW-0017 and REW-0018: the nine inherited, plus `Rewind.Loop.Prelude.Bounds`, `Rewind.Loop.Prelude.Latch`, `Rewind.Loop.AnchorCommit.Boundary`, `Rewind.Loop.BreakSignature.ReadsLoopTime`, `Rewind.Message.Queue.Queueing`, `Rewind.Message.Queue.Ordering`, `Rewind.Message.Queue.Expiry` and `Rewind.Message.Catalog.ResolvesSliceCopy`. Each set of four was verified on its own branch, where the other four did not exist |
