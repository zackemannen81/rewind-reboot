...
| `docs/adr/ADR-0011_asset-source-and-provenance.md` | Accepted, amended twice. Art assets are project-owned or generated by default. The 2026-08-25 amendment corrects the clause that named `/Game/Art/Texture/`: those textures are the owner's own work, and their JPEG sources are tracked at `Rewind/ArtSource/Textures/`, 76 KB. REW-0022 unbound the map from them along with the Fab packs; REW-0025 re-binds them. The 2026-08-27 amendment records that ADR-0013 supersedes points 2 and 4 in part: a third-party asset may now be adopted, and the pack it came from still stays outside the tracked closure |
| `docs/adr/ADR-0012_echo-semantics.md` | Accepted, amended 2026-08-27. Echo is narrative only: nothing samples the player's transform, nothing persists a path, nothing replays a route the player took. The amendment records what Echo may be — authored silent figures performing authored actions, keyed to loop time or a knowledge fact — so the boundary is a rule rather than an argument |
| `docs/adr/ADR-0013_third-party-asset-adoption.md` | Accepted 2026-08-27. A third-party asset may enter the tracked closure when it is free, licensed for commercial use, recorded with provenance and inside the owner's 300 000 triangle ceiling. Only the referenced dependency closure is tracked, never the pack. No asset has been brought up to this record yet; the Fab electrical panel staged during REW-0038 is the first case and still lacks its provenance record |
| `docs/adr/ADR-0014_source-organisation.md` | Accepted 2026-08-27. A C++ class is justified by a new rule, never by a new object; flavour objects become one `ARewindProp`. The eight-directory layout is decided and **not yet applied** — `Rewind/Source/Rewind/` is still flat, and the 83-file move is a separate uncharted task. `ARewindProp` does not exist |
...
| `docs/design/` | Ten accepted documents plus three proposed. `echo.md`, `the-macro-loop.md` and `planted-details.md` are Proposed and may not be cited by a task yet. Ownership in `docs/design/README.md`; `player-messages.md` owns the player-facing text channel and states that the debug overlay is not UI; `tutorial-and-first-run.md` owns first-run copy, the naming rule, and first-time gating |
...
| `Rewind/Source/Rewind/RewindRadio.cpp` | Four channels on the loop clock. The accepted channel speaks `7312` across 20 seconds at phases 4, 9, 14 and 19 of a 50-second cycle. A full sequence grants the stored fact; individual digits remain player memory. Baseline is channel 2 as of 2026-08-27, one step short of the code channel, authored in `chapter-1-authored.md` and registered as a planted detail |
