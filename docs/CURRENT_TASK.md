...
- **Read before working: this frozen charter now conflicts with the design
  it was waiting for.** Its out-of-scope list excludes "branching routes,
  alternate paths and route knowledge", and REW-0005 decided a lift-or-
  stairs choice, which is a branch. A frozen charter is superseded and
  never rewritten, so the decision is whether to supersede this task and
  charter its replacement, or to read the exclusion narrowly as barring
  route knowledge as a progression system rather than one authored branch
  the chain requires. That is not resolved here, and nothing should be
  built until it is.
